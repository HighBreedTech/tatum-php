 ...
 */
function sendEthOffchainTransaction(testnet: boolean, body: TransferEthOffchain, provider?: string) {
 ...
 * @returns transaction id of the transaction in the blockchain
 */
function sendEthErc20OffchainTransaction(testnet: boolean, body: TransferEthErc20Offchain, provider?: string) {
    await validateOrReject(body);
    const {
        mnemonic, index, privateKey, nonce, ...withdrawal
    } = body;
 ...
 */
function signEthOffchainKMSTransaction(tx: TransactionKMS, $fromPrivateKey, testnet: boolean, provider?: string) {
